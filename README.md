...
- Данные товаров централизованы в PostgreSQL (`products`), фронтенд читает каталог только через API `/api/products`.
...
Все admin endpoint требуют заголовок `x-admin-key` (если задан `ADMIN_API_KEY`):

- `GET /api/admin/summary` — сводка по заказам/товарам/обращениям/отзывам
...
- `GET /api/admin/contacts?limit=100` — последние обращения

## Вход в админ-панель

1. Открывайте админку только по ссылке с ключом в query-параметре.
1. После открытия можно сохранить ключ в блоке «Вход», затем нажать «Обновить данные».

Быстрый вход также поддерживается через URL-параметр:

- `admin.html?key=ВАШ_КЛЮЧ`

Без параметра `key` страница `admin.html` возвращает 404.
